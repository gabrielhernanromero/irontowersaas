...
import { ArrowLeft, Camera, XCircle, AlertTriangle, ChevronDown, WifiOff, CheckCircle2 } from 'lucide-react'
...
export default function NuevaNovedadPage({ searchParams }: Props) {
  ...
  const { register, handleSubmit, setValue, getValues, formState: { errors } } = useForm<NuevaNovedadInput>({
  ...
  const handleFoto = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!raw) { setFotoFile(null); setFotoPreview(null); return }
  ...
  async function onSubmit(data: NuevaNovedadInput) {
    ...
    setSubmitting(true)
    try {
      // ── OFFLINE: guardar en IndexedDB y salir ───────────────────────────
      if (!navigator.onLine) {
        await enqueue({
          type:                    'novedad',
          label:                   `Novedad ${data.hora} — ${data.descripcion.slice(0, 40)}`,
          turno_id:                data.turno_id,
          hora:                    data.hora,
          descripcion:             data.descripcion,
          riesgo_detectado:        data.riesgo_detectado || undefined,
          medidas_adoptadas:       data.medidas_adoptadas || undefined,
          observaciones_generales: data.observaciones_generales || undefined,
          es_alerta:               data.es_alerta ?? false,
          es_incidencia:           data.es_incidencia ?? false,
          incidencia_titulo:       data.incidencia_titulo || undefined,
          incidencia_severidad:    data.incidencia_severidad || undefined,
          fotoBlob:                fotoFile ?? undefined,
          fotoName:                fotoFile?.name,
          created_at:              Date.now(),
        })
        setSavedOffline(true)
        setTimeout(() => router.push('/tecnico/libro-guardia?ok=offline'), 1500)
        return
      }
      ...
      // Si perdimos conexión justo antes del fetch, guardar offline
      if (!navigator.onLine) {
  ...
  if (savedOffline) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4 text-center">
        ...
        <div>
          <p className="font-bold text-brand-ink text-lg">Guardado sin conexión</p>
          <p className="text-sm text-gray-500 mt-1">Se enviará automáticamente cuando vuelva la conexión.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="pb-28">
      <div className="flex items-center gap-3 mb-5">
        ...
        {esApoyo && (
          <span className="ml-auto text-xs font-semibold px-2 py-1 rounded-full bg-blue-100 text-blue-700">
            Apoyo
          </span>
        )}
      </div>
      ...
      {typeof window !== 'undefined' && !navigator.onLine && (
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4">
          ...
          <p className="text-xs text-amber-700 font-medium">Sin conexión — se guardará localmente</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        <div>
          <p className="text-sm font-medium mb-2">
            Foto <span className="text-gray-400 text-xs">(opcional · se comprime automáticamente)</span>
          </p>
          {compressing && (
            <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
              <span className="animate-spin">⟳</span> Optimizando imagen...
            </div>
          )}
          {fotoPreview ? (
        ...
        {/* ── Toggle incidencia persistente (encargado y apoyo) ── */}
        <div className={`rounded-xl border-2 transition-colors ${esIncidencia ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-white'}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-gray-500">
                  {esApoyo
                    ? 'Se notificará al encargado y quedará registrada'
                    : 'Se arrastra entre turnos hasta resolverse'}
                </p>
              </div>
            </div>
          ...
          {esIncidencia && (
            <div className="px-4 pb-4 flex flex-col gap-3 border-t border-red-200">
              {esApoyo && (
                <p className="pt-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  El encargado recibirá una notificación con esta incidencia.
                </p>
              )}
              <div className="pt-2">
