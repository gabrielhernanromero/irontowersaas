...
import {
  CheckCircle, Circle, MapPin, Loader2, Trophy, Flag, QrCode,
...
interface Props {
  ...
  historialPorIncidencia: Record<string, HistorialEntry[]>
}
...
function formatFechaHist(iso: string): string {
...
export default function RondaActivaClient({ ronda, puntos, incidenciasPorPunto, historialPorIncidencia }: Props) {
    ...
    scanLoopRef.current = setInterval(() => {
      ...
      if (puntoEsperado && puntoEsperado.codigo_qr !== codigoQr) {
        if (!wrongQrTimer.current) {
          setWrongQr(true)
          wrongQrTimer.current = setTimeout(() => { setWrongQr(false); wrongQrTimer.current = null }, 2000)
        }
        return
      }
  ...
  function handleFotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setPendingConfirm(prev => prev ? { ...prev, fotoFile: file, fotoPreview: URL.createObjectURL(file) } : null)
  }
  ...
  function toggleHistorial(incId: string) {
  ...
  async function registrarScan(
    ...
    try {
      ...
      // Eliminar incidencias cerradas del estado local
      if (json.incidenciasCerradas?.length) {
        const cerradas = new Set<string>(json.incidenciasCerradas)
        setIncByPunto(prev => {
          const next: typeof prev = {}
          for (const [pid, incs] of Object.entries(prev)) {
            next[pid] = incs.filter(i => !cerradas.has(i.id))
          }
          return next
        })
      }
    ...
    <div className="space-y-5">
      ...
      {liveScanning && (
        <div className="fixed inset-0 z-[200] bg-black flex flex-col">
          <video ref={videoRef} playsInline muted onCanPlay={startScanLoop} className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative">
              <div className="w-64 h-64 relative" style={{ boxShadow: '0 0 0 9999px rgba(0,0,0,0.55)' }}>
          ...
          <div className="absolute bottom-0 left-0 right-0 pb-12 flex flex-col items-center gap-3">
            <p className="text-white text-sm font-semibold drop-shadow">Apuntá la cámara al código QR</p>
            <button onClick={closeLiveScanner} className="flex items-center gap-2 bg-white/20 backdrop-blur text-white font-semibold px-6 py-3 rounded-full min-h-[48px]">
              <X size={16} /> Cancelar
            </button>
          </div>
          <button onClick={closeLiveScanner} className="absolute top-4 right-4 bg-black/40 text-white rounded-full p-2 min-w-[44px] min-h-[44px] flex items-center justify-center z-10">
            <X size={20} />
          </button>
        </div>
      )}
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs bg-amber-100 text-amber-700 font-semibold px-2.5 py-1 rounded-full">En curso</span>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
        ...
        <div className="w-full bg-gray-100 rounded-full h-3">
          <div className="h-3 rounded-full bg-brand-orange transition-all duration-500" style={{ width: `${pct}%` }} />
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        {puntos.map((punto, i) => {
          ...
          return (
            ...
            >
              {escaneado  ? <CheckCircle size={24} className="text-emerald-500 shrink-0" />
              : procesando ? <Loader2 size={24} className="text-brand-blue shrink-0 animate-spin" />
      ...
      {/* Sheet confirmación */}
      {pendingConfirm && (() => {
        const incidenciasEnPunto = incByPunto[pendingConfirm.puntoId] ?? []
        ...
        return (
          <>
            ...
            <div className="fixed bottom-0 left-0 right-0 md:left-56 z-[70] bg-white rounded-t-2xl shadow-xl p-5 pb-8 max-h-[90vh] overflow-y-auto">
              ...
              {/* Incidencias activas — acción obligatoria */}
              {incidenciasEnPunto.map(inc => {
                const acc        = incAcciones[inc.id]
                const hist       = historialPorIncidencia[inc.id] ?? []
                const isExpanded = expandedHist.has(inc.id)
                const enSeguimiento = inc.estado === 'en_seguimiento'
                ...
                return (
                  <div key={inc.id} className={`rounded-xl border p-4 mb-3 transition-colors ${
                    ...
                    : enSeguimiento             ? 'bg-amber-50 border-amber-200'
                    :                             'bg-red-50 border-red-200'
                  }`}>
                    <div className="flex items-start gap-2 mb-3">
                      <AlertTriangle size={16} className={`shrink-0 mt-0.5 ${
                        acc?.accion === 'resuelto' ? 'text-emerald-500'
                        : enSeguimiento || acc?.accion === 'cambio' ? 'text-amber-500'
                        : 'text-red-500'
                      }`} />
                      <div className="flex-1 min-w-0">
                        <p className={`text-xs font-bold mb-0.5 ${
                          acc?.accion === 'resuelto' ? 'text-emerald-700'
                          : enSeguimiento ? 'text-amber-700'
                          : 'text-red-700'
                        }`}>
                          {enSeguimiento ? 'En seguimiento' : 'Incidencia abierta'}
                        </p>
                    ...
                    <button
                      type="button"
                      onClick={() => toggleHistorial(inc.id)}
                      className="flex items-center gap-1 text-xs text-blue-600 mb-3 min-h-[36px]"
                    >
                      <ChevronDown size={12} className={`transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} />
                      {isExpanded ? 'Ocultar historial' : `Ver historial (${hist.length + 1} entrada${hist.length !== 0 ? 's' : ''})`}
                    </button>
                      ...
                      <div className="mb-3 space-y-2 border-t border-gray-200 pt-3">
                        ...
                        {hist.map(h => (
                          <div key={h.id} className="flex gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs text-gray-700 break-words">{h.descripcion}</p>
                              <p className="text-xs text-gray-400 mt-0.5">
                                {h.hora ?? formatFechaHist(h.created_at)}
                                {h.autor ? ` · ${h.autor}` : ''}
                              </p>
                            </div>
                          </div>
                        ...
                        {hist.length === 0 && (
                          <p className="text-xs text-gray-400 italic">Sin seguimientos registrados.</p>
                        )}
                      </div>
                    )}
                    ...
                    {(acc?.accion === 'cambio' || acc?.accion === 'resuelto') && (
                      <textarea
                        ...
                        placeholder={acc.accion === 'cambio' ? 'Describí qué cambió...' : '¿Cómo se resolvió? (obligatorio)'}
                        maxLength={500}
                        rows={2}
              ...
              {pendingConfirm.fotoPreview ? (
                <div className="relative mb-4">
                  ...
                  <img src={pendingConfirm.fotoPreview} alt="Foto del punto" className="w-full h-44 object-cover rounded-xl border border-gray-200" />
                  <button
                    type="button"
                    onClick={() => setPendingConfirm(prev => prev ? { ...prev, fotoFile: null, fotoPreview: null } : null)}
              ...
              <button type="button" onClick={cancelarConfirm} className="w-full text-gray-400 text-sm py-3 mt-1 min-h-[44px]">
                Cancelar
              </button>
            </div>
          </>
